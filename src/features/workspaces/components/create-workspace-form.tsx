...
export const CreateWorkspaceForm = ({ onCancel }: CreateWorkspaceFormProps) => {
  ...
  const inviteLinkForm = useForm({
    defaultValues: {
      inviteLink: ''
    }
  });
  const isAdmin = currentUser?.email === 'kashif@admin.com';
  ...
  const handleInviteLinkSubmit = (inviteLink: string) => {
    try {
      ...
      const inviteCode = pathParts[pathParts.indexOf('join') + 1];
    ...
    return (
      <Card className="size-full border-none shadow-none">
        ...
        <CardContent className="p-7">
          <Form {...inviteLinkForm}>
            <form onSubmit={inviteLinkForm.handleSubmit((values) => {
              handleInviteLinkSubmit(values.inviteLink);
            })} className="space-y-4">
              <FormField
                control={inviteLinkForm.control} // Add this
                name="inviteLink"
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
